...
- [ ] (FF6) [reservations] **agentService.createBooking: avoid DOUBLE booking-confirmed notification when requireCardOnFile is ON** — when `confirmReservation` succeeds via `wasCardOnFileDeferred` (requireCardOnFile fleet + card/waiver satisfied) it publishes booking-confirmed events INTERNALLY (`reservationService.js` ~9737); `createBooking` then ALSO fires `sendBookingConfirmation`/`scheduleReminders`/`sendPostBookingRiderEmails` (`agentService.js` ~580), double-sending. Pre-existing (old code fired unconditionally), DORMANT pre-flip (requireCardOnFile off by default), and dedup-mitigated — so deferred from the C6 PR. Fix: mirror the public create-route's publish-ownership gate (`!requireCardOnFile`, load tenantConfig) like `public.js:4195-4197`, OR have `confirmReservation` signal whether it published. **Must land before/at the card-on-file flip.** Minor adjacent: `_agent.summary` hardcodes "Booking confirmed!" even when confirm failed (status pending) — pre-existing cosmetic.
