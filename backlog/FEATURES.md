...
- **Status**: open — **cross-repo build complete, on staging PRs (awaiting merge), prod re-cut pending.** Reservations: #1085 + hardening #1089 merged to staging; **P1 idempotency + #4 late-fee** in PR [reservations#1092](https://github.com/TheLineExp/fleetmanager-reservations/pull/1092) (OPEN → staging). FM V3 UI (Issue Refund on live bookings, credit-default, per-submit idempotency key, bike_damage/injury reasons) in PR [Fleetmanager_V3#1059](https://github.com/TheLineExp/Fleetmanager_V3/pull/1059) (OPEN → staging). Prod promotion #1087 was closed/held; **re-cut both prod releases (reservations→master, FM V3→main) once #1092 + #1059 merge to staging and verify** — ship together. All Codex P1/P2 from the first review addressed.
- **Priority**: high
