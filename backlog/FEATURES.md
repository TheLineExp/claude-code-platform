...
- **Status**: open — **complete + hardened, live on staging, PROD PRs OPEN (awaiting user merge — ship together).** All staging work merged (#1085 + #1089 + #1092 reservations; #1059 FM V3) and verified healthy on staging. Prod releases re-cut as curated cherry-picks: **[reservations#1097](https://github.com/TheLineExp/fleetmanager-reservations/pull/1097) → master** (6 commits; resolved the Connect `refundApplicationFee` cherry-pick conflict — Connect isn't in prod) and **[Fleetmanager_V3#1063](https://github.com/TheLineExp/Fleetmanager_V3/pull/1063) → main** (UI). Both tested green against their prod base. **Merge the two together** (UI sends the idempotencyKey the backend honors). All Codex P1/P2 across all three review rounds addressed. After merge: back-merge master→staging (reservations) + main→staging (FM V3) per convergence rule, then worktree cleanup.
- **Priority**: high
