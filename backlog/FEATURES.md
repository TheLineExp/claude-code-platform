...
- **Status**: PROD PRs OPEN (green) — staging verified, prod-promotion PRs up & CI-green, awaiting user merge. Reservations [#1075](https://github.com/TheLineExp/fleetmanager-reservations/pull/1075)→master + FM V3 [#1052](https://github.com/TheLineExp/Fleetmanager_V3/pull/1052)→main (curated release/*-prod, FR-049-only). Merge reservations first/with FM V3 (editor needs the API). Then Incorporated + worktree cleanup. Prior: Reservations [#1070](https://github.com/TheLineExp/fleetmanager-reservations/pull/1070) (SpecialHours model+migration+availabilityService enforcement+admin CRUD+public `dateOverrides`+FE picker+tests/repro; Codex review addressed — cross-location closures, slot clip, multi-day clamp, reversed-window, calendar grey-out; ship note `docs/changelog/feature-w2-special-hours.md`) + FM V3 [#1047](https://github.com/TheLineExp/Fleetmanager_V3/pull/1047) (Special/Holiday Hours editor panel + multi-location fan-out; error-coercion fix). **Remaining: prod promotion** — reservations via curated `release/*-prod` (staging→master), FM V3 staging→main; then move to Incorporated + clean up worktrees (rule 7: not before prod-verified).
- **Priority**: high
