# graphify — query / path / explain / add

Loaded on demand by the `/graphify` core skill when an existing graph is queried
(`graphify-out/graph.json` present + a natural-language question, or an explicit
`query`/`path`/`explain`/`add` subcommand). Run the interpreter guard first, then the
relevant section. This is the hot path — do NOT run the build pipeline.
