...
4. **Re-review after the fix pass** — re-run `parity-sweep` + `money-concurrency-reviewer`
   (for gated change types) AND `outside-reviewer` on the fix commits (pushing unreviewed
   fix commits is the 8-round-cascade failure mode) — then merge once clean on the new HEAD.
